...
runAutostart(void) {
	...
	system("cd ~/.dwm; bash ./autostart.sh &");
}
...
tile(Monitor *m)
{
	...
	for (i = 0, my = ty = m->gappoh*oe, c = nexttiled(m->clients);
		 c; c = nexttiled(c->next), i++)
	{
		if (i < m->nmaster) {
			r = MIN(n, m->nmaster) - i;  /* <-- add this */
			h = (m->wh - my - m->gappoh*oe - m->gappih*ie * (r - 1)) / r;
			resize(c,
				   m->wx + m->gappov*oe,
				   m->wy + my,
				   mw - (2*c->bw) - m->gappiv*ie,
				   h - (2*c->bw),
				   0);
			my += HEIGHT(c) + m->gappih*ie;
		} else {
			r = n - i;                   /* already present in your else */
